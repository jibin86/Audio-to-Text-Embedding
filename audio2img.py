...
def parse_args():
    ...
    parser.add_argument(
        "--config",
        type=str,
        default="configs/v2-inference-v.yaml",
        help="path to config which constructs model",
    )
    parser.add_argument(
        "--ckpt",
        type=str,
        default="pretrained_models/768-v-ema.ckpt",
...
def main(opt):
    ...
    start_code = None
    # print("opt.fixed_code",opt.fixed_code) # False
    if opt.fixed_code:
        ...
        model.ema_scope():
            ...
            for n in trange(opt.n_iter, desc="Sampling"):
                ...
                # for prompts in tqdm(embeddings_tensor, desc="data"):
                    ...
                    c = torch.cat((uc[:, :1, :], embedding_tensor), dim=1)
                    # print("c", c[:, 0, :], c.shape)
                    # print("c", uc, uc.shape)
                    shape = [opt.C, opt.H // opt.f, opt.W // opt.f]
